...
int main(int argc, char **argv) {
  ...
  while (!queue.empty()) {
    count++;
    auto &[subState, cost] = queue.top();
    if (subState.finished(fieldCostGrid)) {
      break; // found target;
    }
    if (cost.real > visited.value(subState._curCol, subState._curRow)) {
    ...
    //           << " \n";
    auto curCost = cost.real;
    auto newStates =
        subState.getNextPossibleStatesAndCost(fieldCostGrid, visited, curCost);
    queue.pop();
